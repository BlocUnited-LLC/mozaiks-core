...
def _load_capabilities_config() -> dict[str, Any]:
    config_path = os.path.join(os.path.dirname(__file__), "..", "config", "ai_capabilities.json")
    ...
    try:
        ...
        logger.error(f"Invalid ai_capabilities.json: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Invalid ai capabilities config")
