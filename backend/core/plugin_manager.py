...
# Define plugins directory and registry file path
PLUGIN_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "../plugins"))
PLUGIN_REGISTRY = os.path.abspath(os.path.join(os.path.dirname(__file__), "config/plugin_registry.json"))

# Add the plugins directory to sys.path for dynamic imports
...
class PluginManager:
    ...
    def update_registry(self):
        ...
        try:
            ...
            for plugin_name in plugin_dirs:
                ...
                frontend_dir = os.path.join(PLUGIN_DIR, "..", "src", "plugins", plugin_name)
            ...
            os.makedirs(os.path.dirname(PLUGIN_REGISTRY), exist_ok=True)
            ...
            if os.path.exists(PLUGIN_REGISTRY):
                logger.info("Using existing registry file")
                with open(PLUGIN_REGISTRY, "r") as f:
                ...
                with open(PLUGIN_REGISTRY, "w") as f:
                    json.dump(empty_registry, f, indent=4)
    ...
    async def register_plugin_notifications(self, plugin_name):
        ...
        """
        try:
            # Load current settings config
            settings_config_path = os.path.join(os.path.dirname(__file__), "config/settings_config.json")
            with open(settings_config_path, "r") as f:
                settings_config = json.load(f)
            
            # Load notifications config
            notifications_config_path = os.path.join(os.path.dirname(__file__), "config/notifications_config.json")
            try:
                with open(notifications_config_path, "r") as f:
                    notifications_config = json.load(f)
            except (FileNotFoundError, json.JSONDecodeError) as e:
                logger.error(f"Error loading notifications config: {e}")
                notifications_config = {"plugins": {}}
