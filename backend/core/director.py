...
def load_config(filename):
    """
    Loads a JSON config file with caching for improved performance.
    
    Args:
        filename (str): The config filename to load
    ...
    """
    global _last_config_refresh, config_cache
    
    # Return cached config if available and not expired
    current_time = time.time()
    cache_key = f"config:{filename}"
    
    if cache_key in config_cache:
        last_refresh = _last_config_refresh.get(filename, 0)
        if current_time - last_refresh < CONFIG_CACHE_TTL:
            return config_cache[cache_key]
    
    # Load config from file
    config_path = os.path.join(os.path.dirname(__file__), f"config/{filename}")
    try:
        ...
        _last_config_refresh[filename] = current_time
        ...
        logger.error(f"Configuration file {filename} contains invalid JSON.")
        raise HTTPException(status_code=500, detail=f"Configuration file {filename} contains invalid JSON.")
    except Exception as e:
        logger.error(f"Error loading {filename}: {e}")
        raise HTTPException(status_code=500, detail=f"Unexpected error loading {filename}.")
...
async def get_navigation(user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # Get plugin registry
        plugin_registry_path = os.path.join(os.path.dirname(__file__), "config/plugin_registry.json")
        try:
