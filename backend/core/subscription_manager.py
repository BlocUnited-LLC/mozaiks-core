...
class SubscriptionManager:
    def __init__(self):
        ...
        self.subscription_config = self.load_config("subscription_config.json")

    def load_config(self, filename):
        """
        Loads the subscription config file dynamically.
        """
        # Expecting the file in backend/core/config/
        config_path = os.path.join(os.path.dirname(__file__), "config", filename)
        try:
            with open(config_path, "r") as f:
                return json.load(f)
        except FileNotFoundError:
            logger.error(f"❌ {filename} not found! Using default config.")
        except json.JSONDecodeError:
            logger.error(f"❌ Invalid JSON in {filename}. Using default config.")
        except Exception as e:
            logger.error(f"❌ Unexpected error loading {filename}: {e}")
        return DEFAULT_SUBSCRIPTION_CONFIG  # Safe fallback
