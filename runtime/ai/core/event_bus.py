...
def log_subscription_change(data):
    logger.info(f"🔄 User {data['user_id']} changed subscription to '{data['plan']}'")

def log_theme_change(data):
    logger.info(f"🎨 Theme changed to '{data['theme']}'")

def log_plugin_executed(data):
    logger.info(f"🔌 Plugin '{data['plugin']}' executed by user {data['user']}")

# Subscribe default event listeners
event_bus.subscribe("subscription_updated", log_subscription_change)
event_bus.subscribe("theme_changed", log_theme_change)
event_bus.subscribe("plugin_executed", log_plugin_executed)

# Decorator for event-driven functions
def on_event(event_name):
    """
    Decorator to automatically subscribe a function to an event
