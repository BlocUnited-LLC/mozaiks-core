...
useEffect(() => {
    ...
    try {
      ...
      if (reuseChatId) {
        ...
        try {
          ...
          const baseUrl = config?.get?.('api.baseUrl') || 'http://localhost:8080';
          const resp = await fetch(`${baseUrl}/api/chats/exists/${currentAppId}/${wfName}/${reuseChatId}`);
          if (resp.ok) {
  ...
  }, [currentChatId, activeGeneralChatId, conversationMode, currentArtifactContext]);
  ...
  const handleAgentAction = async (action) => {
    ...
    try {
      ...
      if (action.type === 'ui_tool_response') {
        ...
        try {
          const uiToolBaseUrl = config?.get?.('api.baseUrl') || 'http://localhost:8080';
          const response = await fetch(`${uiToolBaseUrl}/api/ui-tool/submit`, {
            method: 'POST',
            headers: {
